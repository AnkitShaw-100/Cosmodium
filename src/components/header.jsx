        ...
        <nav className="py-3 flex items-center px-4 sm:px-6 md:px-12 lg:px-20 xl:px-28 2xl:px-32">
            <div className="flex items-center flex-1">
                <Link to="/" className="flex items-center">
                    <img src="/Logo.png" className="h-12 md:h-16 pl-0 md:pl-4" alt="Cosmodium Logo" />
                </Link>
            </div>
            ...
            <div className="md:hidden ml-2 relative">
                ...
                >
                    <svg className="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d={open ? "M6 18L18 6M6 6l12 12" : "M4 6h16M4 12h16M4 18h16"} /></svg>
                </button>
                ...
                {open && (
                    <div className="absolute right-0 mt-2 w-56 bg-gray-900 border border-white/10 rounded-md shadow-lg p-3 z-50">
                        {!isAuthenticated ? (
                            <div>
                                <button onClick={() => { setOpen(false); navigate('/auth') }} className="w-full text-left px-3 py-2 bg-white/5 rounded hover:bg-white/10">Login</button>
                            </div>
                        ) : (
                            <div className="space-y-2">
                                <Link to="/dashboard" onClick={() => setOpen(false)} className="flex items-center gap-2 px-3 py-2 rounded hover:bg-white/5">
                                    <LinkIcon className="h-4 w-4" />
                                    Dashboard
                                </Link>
                                <button
                                    onClick={async () => {
                                        try { await signOut(); } finally { setOpen(false); navigate('/') }
                                    }}
